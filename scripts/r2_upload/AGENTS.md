...
- row manifest 在每个图片 `items[]` 上写入可选 `blurhash`，与该 item 的 thumb/display 描述一起发布；public、`auth_sfw`、`auth_nsfw` 仍先按 category 过滤，不能让 NSFW BlurHash 进入 SFW manifest。manifest 缺该字段时由 Web 端兼容为 `null`。
