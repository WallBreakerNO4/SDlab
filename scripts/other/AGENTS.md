...
- 本目录放轻量离线脚本：把 prompt CSV 转成 YAML 资产，供 `data/` 与生图脚本消费；文件名保留 `json` 后缀，实际输出是 YAML。
- 例外：`backfill_run_style_items.py` 用于回填 `run_style_items`，会读 Supabase `runs` 并幂等 upsert；除此之外本目录不引入数据库逻辑。
...
- 输出 payload 形态固定为 `schema + items`；Y 轴 prompt 使用 `prompt-y-table/v3`，tag 类型写在 `tags[].type`。

## 反模式

- 不要把转换输出写进 `outputs/` 或其他运行目录。
- 不要在这里引入生图、上传、认证或数据库逻辑（唯一例外是 `backfill_run_style_items.py` 回填脚本）。
- 不要被文件名误导去改成 JSON；仓库资产格式以 YAML 为准。
