...
- 37 个测试文件；覆盖 generation runner、R2 上传、CLI 菜单、重试机制、合约校验。偏"可观测输出"：验证文件生成、JSON 字段合约、结构化错误。
...
| prompt 资产 | `test_prompt_assets.py` | data/ 下 JSON 有效性 |
...
- fixtures 在 `tests/fixtures/` 目录；共享 fixture 用 `conftest.py`
