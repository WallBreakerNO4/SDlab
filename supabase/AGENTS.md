...
| RPC 性能验收           | `tests/style_comparison_rpc_explain.sql` | 用真实 viewer fixture 验证 1×1、1×6、40×12，并校验最大 slice 的 40/480/12 计数 |
...
- `get_style_comparison_slice` 只授予 `authenticated`，必须保持 `SECURITY INVOKER`、显式 `(select auth.uid())`、函数内 40 style / 12 run 上限和固定异常文案；不得接受客户端 `user_id` 参数。
