...
- 本目录放 Web 侧服务端 route：一类是 ComfyUI JSON 查询 API，一类是 R2 私有对象代理；子目录只补充各自 payload / 代理细节。
...
| R2 代理细则      | `app/api/r2/AGENTS.md`                 | private bucket 代理、range / HEAD         |
...
- 进入查询或代理前先校验输入：`runDir` 先做 type guard，`r2Key` 先做 segment 校验；不要在 route 内自由拼接路径或对象 key。
- 子目录文档优先级高于本文件：`comfyui/` 讲 payload 收敛，`r2/` 讲对象代理语义。

## 反模式

- 不要在 route 中直接 new 裸 `createServerClient()`（PKCE callback 特例除外，但它在 `app/auth/`）。
- 不要在 API payload 里直接返回整包数据库对象或上游异常。
- 不要绕过 `lib/comfyui-path.ts` / `lib/r2-url.ts` 手拼路径或代理 URL。
