...
const evidencePath = ".sisyphus/evidence/task-auth-03-range.txt"
...
> {
  ...
  const email = `e2e-task-auth-range-${Date.now()}-${randomUUID()}@example.com`
...
test("task-auth-03 private proxy supports Range 206", async ({ context, page }, testInfo) => {
  ...
  const rangeResponse = await context.request.get(originalDownloadUrl, {
    headers: {
      Range: "bytes=0-15",
    },
  })
  ...
  if (rangeResponse.status() !== 206) {
    const bodyText = await rangeResponse.text()
    assertNoSensitiveLeak(bodyText)
  }

  expect(rangeResponse.status()).toBe(206)

  const headers = rangeResponse.headers()
  expect(headers["content-range"] ?? "").toMatch(/^bytes 0-15\/[0-9]+$/)

  const body = await rangeResponse.body()
